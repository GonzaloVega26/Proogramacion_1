...
funcion vectorDin(real) diferenciaDeVectores(vectorDin(real) A, vectorDin(real) B){
	...
	variarMas1(i,0,tamanio(A)-1)
		variarMas1(j,0,tamanio(B)-1)
			...
			finSi
		finVariar
		si(NO bandera) agregaEleVDin(vectorDiferencia,A[i]); // si la bandera quedo en falso despues de toda la iteracion
...
vectorDin(real) C = diferenciaDeVectores(conjuntoAsinDups,conjuntoBsinDups);
mostrar << "VECTOR DIFERENCIA" << salto;
mostrarVector(C);
